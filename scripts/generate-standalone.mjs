...
let saved={};try{saved=JSON.parse(localStorage.getItem("ai-compiler-standalone")||"{}")||{}}catch{}let completed=saved.completed||{},startDate=saved.startDate||"2026-08-10",active=0,query="";const total=DATA.weeks.length*7;
...
function render(){const list=DATA.weeks.map((w,i)=>({...w,index:i+1})).filter(w=>(!active||w.phase===active)&&JSON.stringify(w).toLowerCase().includes(query.toLowerCase()));weeks.innerHTML=list.map(w=>{const tasks=[...w.days,"恢复、补漏或休息；若本周已完成，做 30 分钟轻量回顾后彻底离开屏幕。"],wd=tasks.filter((_,d)=>completed[w.index+"-"+(d+1)]).length;return '<article class="week" data-week="'+w.index+'"><button class="summary"><span class="wn">W'+String(w.index).padStart(2,'0')+'</span><span><small>阶段 '+w.phase+' · '+dateLabel(w.index,0)+'—'+dateLabel(w.index,6)+'</small><h2>'+w.title+'</h2><p>'+w.goal+'</p></span><span class="score">'+wd+'/7</span></button><div class="body"><div class="output"><span>本周验收物</span><b>'+w.output+'</b></div><div class="days">'+tasks.map((t,d)=>{const id=w.index+'-'+(d+1),done=!!completed[id];return '<label class="day '+(done?'done':'')+'" data-id="'+id+'"><input type="checkbox" '+(done?'checked':'')+'><span class="check">'+(done?'✓':'')+'</span><span class="date">DAY '+String(d+1).padStart(2,'0')+'<span>周'+dayNames[d]+' · '+dateLabel(w.index,d)+'</span></span><span class="task">'+t+'<small>'+timePlan[d]+'</small></span></label>'}).join('')+'</div></div></article>'}).join('')||'<div class="empty">没有匹配的周计划</div>';weeks.querySelectorAll('.summary').forEach(b=>b.onclick=()=>b.parentElement.classList.toggle('open'));weeks.querySelectorAll('.day').forEach(el=>el.onchange=()=>{completed[el.dataset.id]=!completed[el.dataset.id];persist();updateProgress();render()})}
document.getElementById('startDate').value=startDate;document.getElementById('startDate').onchange=e=>{startDate=e.target.value;persist();render()};document.getElementById('search').oninput=e=>{query=e.target.value;render()};renderPhases();render();updateProgress();
</script></body></html>`;
