...
const plannedWeeks = rawWeeks.map((week, index) => ({
  ...week,
  sourceWeek: index + 1,
  fileHint: weekFileHints[index + 1] || "",
  knowledge: weekKnowledgePoints[index + 1] || [],
}));
...
<title>AI 编译器学习日志｜50 周行动计划</title><style>
...
<header class="top"><div class="brand"><span class="mark">AC</span><span>AI COMPILER<br><b>LEARNING LOG</b></span></div><div class="progress"><span id="doneTop">0/350</span><div class="bar"><i id="topBar"></i></div><b id="percentTop">0%</b></div></header>
<section class="hero"><div><span class="eyebrow">350 天 · 50 周 · 2 个简历主项目 + 1 次开源协作</span><h1>AI 编译器<br><em>学习日志</em></h1><p>从 2026 年 8 月末到 2027 年 8 月：每一天完成一个边界清楚的工程任务，并留下可运行代码、测试、IR 或性能证据。12 月起每周固定 5 道 LeetCode；项目 A 于 3—5 月完成，项目 B 于 5—7 月完成，随后留出质量审计、开源协作和秋招演练时间。</p></div><div class="stat"><span class="eyebrow">计划进度</span><strong id="percentBig">0%</strong><span id="doneBig">已完成 0 个任务</span><label>计划开始日 <input id="startDate" type="date" value="2026-08-31"></label></div></section>
...
function leetcodeActive(w){const x=new Date(startDate+"T00:00:00");x.setDate(x.getDate()+(w.index-1)*7);return x>=new Date("2026-12-01T00:00:00")}function weeklyClosure(w){return "周验收：在干净环境从零运行「"+w.output+"」；核对本周测试、代表性输入和一项已知限制，并把复现命令写入日志"}function tasks(w){const core=[...w.days.slice(0,6),weeklyClosure(w)],algo=["；LeetCode 1 题：写复杂度和 2 个边界","","；LeetCode 1 题：口头复述思路","","；LeetCode 1 题：补一个替代解法","","；LeetCode 2 题：整理模板和错因"];return core.map((task,index)=>task+(leetcodeActive(w)?algo[index]:""))}function resources(w){return DATA.weekLearningResources.find(x=>x.week===(w.sourceWeek||w.index))||{videos:DATA.phaseResources[w.phase-1],references:DATA.phaseReferences[w.phase-1]}}function picks(items,d){return Array.from({length:Math.min(2,items.length)},(_,offset)=>items[(d+offset)%items.length])}
function workspace(w){if(w<=12)return"ai-compiler-year-one/projects/minitensor";if(w<=20)return"ai-compiler-year-one/projects/cuda-kernels";if(w<=29)return"ai-compiler-year-one/projects/compiler-playground";if(w<=35)return"ai-compiler-year-one/projects/gpu-operator-lab";if(w<=44)return"ai-compiler-year-one/projects/transformer-compiler";if(w<=47)return"ai-compiler-year-one/projects/mlir-toy-lab";return"ai-compiler-year-one/portfolio"}function command(w){if(w<=12)return"cmake -S . -B build -DCMAKE_BUILD_TYPE=Debug\\ncmake --build build -j\\nctest --test-dir build --output-on-failure";if(w<=20)return"cmake -S . -B build -DCMAKE_BUILD_TYPE=Release\\ncmake --build build -j\\nctest --test-dir build --output-on-failure";if(w<=29)return"python -m pytest -q\\npython examples/run_week.py --week "+w;if(w<=35)return"python -m pytest -q\\npython benchmarks/run.py --quick";if(w<=44)return"python -m pytest tests -q\\npython benchmarks/e2e.py --quick";if(w<=47)return"cmake --build build --target check-mlir\\nbuild/bin/mlir-opt <当天的 .mlir 文件> --verify-diagnostics";return"git status --short\\ngit diff --check"}function esc(x){return String(x).replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;")}function guide(w,d,t){const ws=workspace(w.index),files=w.fileHint,note="docs/learning-log/W"+String(w.index).padStart(2,"0")+"-D"+String(d+1).padStart(2,"0")+".md",main=t.split("；LeetCode")[0],modes=["定义输入、输出和接口边界","实现最小正确版本","用正常与边界输入验证","定位一次中间状态或错误","记录可比较的结果","专题学习并复写最小示例","在干净环境复现本周交付"];const steps=["今天的唯一任务："+main,"按「"+modes[d]+"」执行；只修改与该任务直接相关的文件："+files,"在 "+note+" 记录本次输入、实际输出、一个失败现象（或风险）及其原因。","运行下方命令；若失败，只修复本任务相关问题后再运行。"+(t.includes("LeetCode")?" 完成算法题并写时间/空间复杂度。":"")];const done=["能从零运行今天产物，并得到可核对的输出、测试结果或 IR。","能用一句话说明：今天改动解决了什么具体问题。","当天日志含输入、输出、命令和一个边界/失败情况。",t.includes("LeetCode")?"LeetCode 题目记录了思路、复杂度和边界。":"代码与验收结果一致后再勾选今天。"];return{ws,files,steps,cmd:command(w.index),done}}
...
(() => {
  ...
  coverage.innerHTML = '<small>近期 AI 编译器 / AI Infra JD → 学习证据</small><h2>岗位要求不是清单，而是每一项都要有能展示的证据</h2><ul><li><b>C++、Linux、构建、内存与性能：</b>W01–W12 的 MiniTensor CPU Runtime、分配器、测试和可复现 benchmark。</li><li><b>CUDA、GEMM、低精度、Profiling：</b>W13–W20 的 kernel 实验；W16 显式加入 FP16/BF16、Tensor Core/CUTLASS 路径。</li><li><b>图编译、IR、Pass、Runtime：</b>W21–W29 走 PyTorch/ONNX/torch.compile；W36–W44 完成 Relax/TensorIR、融合、动态 shape、runtime 和 fallback。</li><li><b>大模型推理：</b>W33、W41、W42 覆盖 prefill/decode、KV Cache、Flash/Paged Attention、量化取舍与 vLLM/PyTorch 接入。</li><li><b>工程质量与协作：</b>每周从干净环境复现；W44 上游 PR；W48–W50 做复现审计、简历和模拟面试。</li><li><b>分布式训练的诚实边界：</b>W21/W26 做 DDP、FSDP 的最小语义实验；这足够回答原理，不等于千卡集群实践。想投训练 Infra 时需另做集群训练/通信项目。</li></ul>';
  ...
  render = function() {
    ...
    weeks.querySelectorAll(".day").forEach(day => {
      ...
      if (!start) return;
      start.insertAdjacentHTML("afterend", '<div class="contractCard"><span>今天具体要学什么</span><div class="contractGrid"><div class="contractRow"><b>核心知识</b><em>' + esc(topic) + '</em></div><div class="contractRow"><b>你要会说</b><em>定义、输入输出、一个边界条件，以及它为何服务于本周模块。</em></div><div class="contractRow"><b>你要会做</b><em>' + esc(task.split("；LeetCode")[0]) + '</em></div></div></div>');
